...
def _generate_calibration_load_sch(spec: CircuitSpec, root_uuid: str, gen_date: str) -> str:
    """Generate KiCad 10 schematic for a precision 1-port 50-ohm RF calibration load standard."""
    u_j1 = str(uuid.uuid4())
    u_r1, u_r2 = str(uuid.uuid4()), str(uuid.uuid4())
	...
	(title_block
		...
		(comment 2 "R1 || R2 = 50.0 Ohm Precision Thin-Film Shunt Termination | Return Loss S11 < -30 dB")
	)
	(lib_symbols
{_get_common_lib_symbols()}
	)
	...
	(symbol (lib_id "Device:R") (at 78.74 101.6 0) (uuid "{u_r2}")
	...
	(symbol (lib_id "power:GND") (at 71.12 119.38 0) (uuid "{u_gnd1}")
		(property "Reference" "#PWR01" (at 71.12 123.19 0) (effects (hide yes)))
		(property "Value" "GND" (at 71.12 124.46 0))
	)
	(wire (pts (xy 38.1 76.2) (xy 71.12 76.2)))
	(wire (pts (xy 71.12 76.2) (xy 63.5 76.2)))
	...
	(text "50Ω Precision 1-Port RF Calibration Load Standard (SOLT Match)\\nR1 || R2 = 50.0Ω | Ultra-Low Inductance Shunt Termination\\nReturn Loss S11 < -30 dB (DC to {spec.f_max_ghz:.0f} GHz)" (at 60.0 135.0 0) (effects (font (size 2.0 2.0))))
)
"""
