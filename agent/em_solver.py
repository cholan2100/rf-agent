...
def _solve_attenuator_s_params(spec: CircuitSpec, freqs_ghz: np.ndarray):
    """
    Calculates physical multi-frequency S-parameter response for 10dB Pi-attenuator
    including microstrip line dispersion, FR4 dielectric loss tangent, and 0805 parasitic inductance/capacitance.
    """
    # Nominal 10 dB attenuator component values
    # R1 = R3 = 96.2 Ohm, R2 = 71.2 Ohm
    r_shunt = 96.2
    r_series = 71.2
    z0 = spec.z0_ohm
    ...
    # [A B; C D] = [1 0; Y1 1] * [1 Z2; 0 1] * [1 0; Y3 1]
    # A = 1 + Z2*Y3
    # B = Z2
    # C = Y1 + Y3 + Y1*Z2*Y3
    # D = 1 + Y1*Z2
    ...
    # Add transmission line phase delay for 2 x 8mm 50-ohm lines
    # Beta = omega * sqrt(eps_eff) / c
    c_light = 3e8
    eps_eff = (spec.dielectric_er + 1.0) / 2.0 + ((spec.dielectric_er - 1.0) / 2.0) / math.sqrt(1.0 + 12.0 * spec.substrate_height_mm / spec.microstrip_width_mm)
    v_phase = c_light / math.sqrt(eps_eff)
    t_line_len = 0.016  # 16 mm total microstrip
